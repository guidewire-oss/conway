...
export async function checkForecastRegistrations(page,base,holdAnswer){
 ...
 await page.getByText('Registered models',{exact:true}).click();
 const status=page.locator('[data-registration-status]');await status.getByText(/Choose the later/).waitFor();
 ...
 await page.locator('[data-register-model]').focus();await page.keyboard.press('Enter');await status.getByText(/Model registered/).waitFor();
 ...
 await page.locator('[data-registration-refresh]').click();await status.getByText(/Choose the later/).waitFor();assert.equal(await page.locator('[data-registration-list] article').count(),2);
 ...
 await page.getByText('Registered models',{exact:true}).click();await status.getByText(/No registered models/).waitFor();await consume(held);
 ...
 const docs=await page.request.get(base+'/docs.html');assert.match(await docs.text(),/id="forecast-registration"/);
}

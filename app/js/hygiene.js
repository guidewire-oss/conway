...
function render(state) {
  ...
  document.getElementById('hygiene-table').innerHTML = `
    <thead><tr>
    <th>Pod${hlp('The team. Click a row to drill: category → the individual Jira issues to fix.', 'the team column')}</th>
    <th>Hygiene${hlp('Composite 0–100% of the measurable signals in this row. Phoenix Project: making work visible is the first step — low hygiene means the model (and you) are flying partly blind.', 'the hygiene score')}</th>
    <th>Sized${hlp('% of recent epic child tasks carrying an estimate (of N sampled). Unsized work forces forecasts to fall back to the pod\'s median item, widening every date.', 'estimate coverage')}</th>
    <th>Median pts${hlp('This pod\'s median story-point value. Used to normalize sizes per pod, so a "5" from a days-pointing team and a complexity-pointing team each map to that team\'s own scale — teams need not agree on units, only be self-consistent.', 'normalized story points')}</th>
    <th>Stale WIP &gt;14d${hlp('% of in-progress items untouched for >14 days. The Goal: this is hidden inventory — it inflates WIP and queue heat and pollutes the freeze list while looking like progress.', 'stale work')}</th>
    <th>Unassigned WIP${hlp('% of in-progress items with no assignee — zombie work nobody owns. The model otherwise counts it as load the team isn\'t really carrying.', 'unassigned work')}</th>
    <th>Link density${hlp('Issues with a blocking link ÷ resolved issues. Conway: low density means real cross-team coupling is invisible in the data, so the dependency graph and suggestions under-count it.', 'dependency coverage')}</th>
    <th>Fix first${hlp('Auto-flags pointing at the cheapest high-impact cleanup for this pod.', 'which issue to fix first')}</th></tr></thead>

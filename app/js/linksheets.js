...
export async function openLinkedSheets(planID, onApplied = async () => {}) {
  ...
  const frame = (title, body) => {
    ...
    box.innerHTML = `<div class="modal-head"><h2 id="linked-sheets-title" tabindex="-1">${esc(title)}</h2><button class="btn btn-secondary" type="button" data-close>${icon('close')}Close</button></div><div data-sheet-body>${body}</div><p data-sheet-status role="status" aria-live="polite"></p>`;
    ...
    frame('Linked Google Sheets', `<p>Link one sheet range for this plan’s teams and one for its initiatives. Changes are captured for review; saved agreements stay unchanged.</p>
      ...
      <div data-sources>${sources.length ? sources.map(source => `<section class="linked-source" style="border-top:1px solid var(--border,#ccc);padding:1rem 0" data-source="${esc(source.id)}">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:.5rem">
          <button class="btn btn-secondary" type="button" data-history>Captured versions</button>
          ${source.status !== 'disconnected' ? `<button class="btn btn-secondary" type="button" data-check ${!result.configured ? 'disabled' : ''}>Check now</button><button class="btn btn-secondary" type="button" data-pause>${source.status === 'paused' ? 'Resume checks' : 'Pause checks'}</button><button class="btn btn-secondary" type="button" data-disconnect>Disconnect</button>` : ''}
        </div>
        ${source.status !== 'disconnected' ? `<form data-settings style="display:flex;gap:.5rem;align-items:end;flex-wrap:wrap;margin-top:1rem"><label>Update mode<select class="form-select" name="mode"><option value="review" ${source.mode === 'review' ? 'selected' : ''}>Review every update</option><option value="auto_apply" ${source.mode === 'auto_apply' ? 'selected' : ''}>Apply safe updates automatically</option></select></label><label>Check interval (minutes)<input class="form-control" name="pollMinutes" type="number" min="5" max="1440" required value="${esc(source.pollMinutes)}"></label><button class="btn btn-secondary" type="submit">Save source settings</button></form>` : ''}
      ...
      <label>Planning input<select class="form-select" name="kind"><option value="teams">Team roster</option><option value="initiatives">Initiatives</option></select></label>
      <label>Google Sheets link<input class="form-control" name="spreadsheetUrl" type="url" required placeholder="https://docs.google.com/spreadsheets/d/…/edit"></label>
      <label>Tab name or A1 range<input class="form-control" name="range" required maxlength="300" placeholder="Teams or 'Planning inputs'!A1:Z500"></label>

...
window.addEventListener('conway:feature-opened', event => {
  const target = ({ guide: 'docs-btn', timeline: 'view-timeline', assistant: 'view-assistant', forecast: 'view-forecast', ready: 'view-ready', execution: 'view-execution', 'linked-sheets': 'plan-linked-sheets', 'plan-sample':'plan-init-sample', snapshots:'obs-snapshots' })[event.detail?.action];
  ...
  if (authMode() === 'auth' && !authGameID() && authUser()) {
    announcements = mountAnnouncements({ request: authFetch,
      ...
      onAction: async action => {
        ...
        return openPlanDestination(action.target === 'view-timeline' ? 'timeline' : action.target === 'view-forecast' ? 'forecast' : action.target === 'view-assistant' ? 'assistant' : action.target === 'view-execution' ? 'execution' : action.target === 'view-ready' ? 'ready' : 'linked-sheets');
      },
    });

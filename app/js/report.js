...
import { activeBaseline } from './baseline.js';
...
// total initiatives will not finish inside the period.
export function fitSentence(sched) {
  const inits = sched.initiatives || [];
  const doomed = inits.filter((i) => PROBLEM_VERDICTS.includes(i.verdict));
  if (!inits.length) return 'No initiatives are scheduled yet.';
  if (!doomed.length) return `All ${inits.length} initiatives commit inside the period.`;
...
export function verdictSectionHTML(sched) {
  ...
  return `<h3>Verdicts</h3><ul class="report-list">${rows.join('') || '<li>no schedule</li>'}</ul>`;
}
...
export function capacitySectionHTML(sched) {
  ...
  const line = (p) => `<li><b>${esc(p.pod)}</b> ρ ${p.rho.toFixed(2)} · ${p.tracks} track${p.tracks > 1 ? 's' : ''}${p.drum ? ' · <b>drum</b>' : ''}</li>`;
...
export function remediesSectionHTML(data) {
  ...
  const remedies = [...(data.remedies || [])]
    ...
    .slice(0, 3);
  if (!remedies.length) return `<p class="report-ok">No remedies — the engine sees nothing worth pulling.</p>`;
  return `<ul class="report-list">${remedies.map((r) => {
    const victims = (r.affectedInitiatives || []).length;
    return `<li><b>${esc(remedyKindLabel(r.kind))}</b> — ${esc(r.target)} → ${esc(remedyLabelOf(r.resultingVerdict))}, portfolio ${fmtDelta(r.objectiveDelta)}${victims ? `, moves ${victims} other initiative${victims > 1 ? 's' : ''}` : ''}</li>`;
  }).join('')}</ul>`;
}
...
export function healthReportHTML(sched, opts = {}) {
  ...
  return `<div class="report-card">
    ...
    <div class="report-foot">
      <p>${active
        ? `Baseline: <b>${esc(active.name)}</b>, saved ${new Date(active.savedAt * 1000).toLocaleDateString()}.`
        : 'No baseline saved — save one to compare future re-plans against.'}</p>
      <p class="hint">Dispatch rule: ${esc(sched.rule || '?')} · portfolio objective ${sched.objectiveScore ?? '?'} · generated ${opts.generatedAt || ''}</p>
    </div>
  </div>`;
}

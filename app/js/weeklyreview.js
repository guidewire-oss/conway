...
export function reviewSummaryHTML(review) {
  ...
  const entries = (items, empty) => items?.length ? `<ul class="ps-3">${items.map(item => `<li class="mb-2">${item.initiative ? `<b>${esc(item.initiative)}</b>: ` : ''}${esc(item.reason)}${item.team ? ` <span class="tag badge text-bg-secondary text-wrap text-start">${esc(item.team)}</span>` : ''}</li>`).join('')}</ul>` : `<p class="hint">${empty}</p>`;
...
export function actionCardsHTML(actions) {
  return (actions || []).map(a => `<article class="execution-decision py-3 border-top" data-action-id="${esc(a.id)}"><h4 class="fs-6 text-body">${esc(a.action)} <span class="tag badge text-bg-secondary text-wrap text-start">${esc(labels[a.status] || 'Open')}</span></h4><p>${esc(a.owner)} · review ${esc(a.reviewDate)}${a.initiative ? ` · ${esc(a.initiative)}` : ''}</p><p>${esc(a.rationale)}</p><small>Recorded ${esc(time(a.createdAt))} by ${esc(a.createdBy)} · snapshot ${esc(a.snapshotId || 'not selected')} · baseline ${esc(a.baselineId || 'none')}</small>

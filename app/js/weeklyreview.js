...
export function mountWeeklyReview(host, {plan, json, live, getContext, ready = true}) {
  ...
  const initialDate=route.get('reviewDate')||dateToday(),initialZone=route.get('reviewTimezone')||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';
  const initiativeNames=(plan.initiatives||[]).map(i=>i.name);
  ...
  host.innerHTML = `<section class="card p-3 panel-card weekly-review my-3"><div class="weekly-heading d-flex flex-wrap justify-content-between align-items-start gap-3"><div><h3 class="fs-5 text-body">Weekly execution review</h3><p class="hint">Prepare the agenda, follow up actions, then save the conclusion your team agreed.</p></div><button class="btn btn-link usage-link text-wrap" data-anchor="weekly-review" type="button">Review guide</button></div>
    <div class="weekly-controls row g-3 align-items-end mb-3"><label class="form-label d-grid gap-1 mb-0 col-12 col-md">Review date <input class="form-control" id="weekly-review-date" type="date" required value="${esc(initialDate)}"></label><label class="form-label d-grid gap-1 mb-0 col-12 col-md">Timezone <input class="form-control" id="weekly-review-timezone" required value="${esc(initialZone)}" autocomplete="off"></label><label class="form-label d-grid gap-1 mb-0 col-12 col-md">Review initiative <select class="form-select" id="weekly-review-initiative"><option value="">All initiatives</option>${initiativeNames.map(name=>`<option value="${esc(name)}" ${route.get('reviewInitiative')===name?'selected':''}>${esc(name)}</option>`).join('')}</select></label><div class="col-12 col-md-auto d-grid"><button type="button" class="btn btn-primary text-wrap" data-weekly-preview ${ready ? '' : 'disabled'}>Prepare review</button></div></div>
  ...
  const initiative=host.querySelector('#weekly-review-initiative');
  const inputs = () => {const context=getContext();return {reviewDate:date.value,timezone:zone.value,...context,filters:{...context.filters,initiative:initiative.value}};};
  ...
  initiative.addEventListener('change',invalidate); date.addEventListener('change',invalidate); zone.addEventListener('input',invalidate);

...
function slicesHTML(slices, start) {
  return `<div class="execution-table-wrap table-responsive border rounded my-3" tabindex="0" aria-label="Team execution evidence, scroll horizontally for all measures"><table class="wip-table table table-hover align-top mb-0 text-wrap"><thead><tr><th scope="col">Team</th><th scope="col">Completed children</th><th scope="col">Agreed start / finish</th><th scope="col">Inferred start / actual finish</th><th scope="col">Start / finish variance</th><th scope="col">Elapsed-time estimate variance</th><th scope="col">Estimated remaining / conditional finish</th><th scope="col">Buffer used</th><th scope="col">Evidence</th></tr></thead><tbody>${slices.map(s => `<tr>
    ...
    <td>${esc(dateWeek(s.actualStartWeek,start))}<small class="d-block text-body-secondary">${Number.isFinite(s.actualStartWeek) ? s.startInferred ? "Inferred from issue activity" : "Start provenance not supplied" : "Start not measured"}</small><br>${esc(dateWeek(s.actualFinishWeek,start))}<small class="d-block text-body-secondary">${Number.isFinite(s.actualFinishWeek) ? "Resolution timestamps in snapshot" : "Finish not measured"}</small></td>
...
export function executionEvidenceHTML(data, {team = '', plan = {}} = {}) {
  ...
  return `<div class="execution-evidence">
    ...
    ${initiatives.map((it, index) => `<article class="card p-3 panel-card execution-initiative my-3">
      ...
      ${(it.originalScopeSlices || []).length ? `<details><summary class="py-2">Original agreed scope</summary>${slicesHTML(it.originalScopeSlices.filter(s=>!team || s.pod === team),start)}</details>` : ''}
      <details><summary class="py-2">Epic bindings · ${esc((it.epicKeys || []).join(', ') || 'None')}</summary>
        <form class="execution-binding d-flex flex-column gap-2 py-3" data-initiative="${esc(it.name)}"><label class="form-label mb-0" for="epic-keys-${index}">Confirmed Jira epic keys</label>
          <input class="form-control" id="epic-keys-${index}" name="epicKeys" value="${esc((it.epicKeys || []).join(', '))}" placeholder="PROJ-123, PROJ-456" autocomplete="off">
          <p class="hint">Comma-separated keys. Saving changes the working plan; the agreed baseline retains its original scope.</p>
          ${(it.suggestions || []).length ? `<p>Suggested name matches — select and save to confirm:</p>${it.suggestions.map(s => `<label class="execution-suggestion form-check mb-0"><input class="form-check-input" type="checkbox" name="suggestion" value="${esc(s.key)}">${esc(s.key)} — ${esc(s.summary)}</label>`).join('')}` : ''}
          <button class="btn btn-secondary text-wrap align-self-start" type="submit">${icon('save')} Save confirmed bindings</button><p class="execution-binding-status" role="status" aria-live="polite"></p>
        </form>
      </details>
    </article>`).join('') || '<p>No initiatives have evidence for this team. Clear the team filter to see untracked initiatives.</p>'}
    <details class="card p-3 panel-card"><summary class="py-2">Order adherence and calibration</summary>
      ...
      <div class="execution-table-wrap table-responsive border rounded my-3" tabindex="0" aria-label="Calibration table, scroll horizontally"><table class="wip-table table table-hover align-top mb-0 text-wrap"><thead><tr><th scope="col">Team</th><th scope="col">Observed duration / estimate</th><th scope="col">Samples</th></tr></thead><tbody>${(data.calibration || []).map(c => `<tr><td>${esc(c.pod)}</td><td>${num(c.factor,'×')} (inferred)</td><td>${num(c.sampleCount)}</td></tr>`).join('') || '<tr><td colspan="3">No comparable completed slices.</td></tr>'}</tbody></table></div>
    </details>
  </div>`;
}

export function decisionsHTML(decisions) {
  return (decisions || []).map(d => `<article class="execution-decision border-top py-3"><h4 class="fs-6 text-body">${esc(d.action)}</h4><p>${esc(d.owner)} · review ${esc(d.reviewDate)}${d.initiative ? ` · ${esc(d.initiative)}` : ''}</p><p>${esc(d.rationale)}</p><small class="d-block text-body-secondary">Recorded ${esc(when(d.createdAt))} by ${esc(d.createdBy)} · snapshot ${esc(d.snapshotId || 'not selected')} · baseline ${esc(d.baselineId || 'none')}</small></article>`).join('') || '<p class="hint">No decisions recorded yet.</p>';
}
...
export async function mountExecution(host, {plan, request, onBindingsSaved, onImport, onAgreement, onSnapshot, onTeam}) {
  ...
  host.innerHTML = `<section class="execution-review"><div class="row-actions row g-3 align-items-end mb-3"><label class="form-label d-grid gap-1 mb-0 col-12 col-lg">Execution snapshot <select class="form-select" id="execution-snapshot" disabled><option value="">Loading snapshots…</option></select></label><div class="col-12 col-md-auto d-grid"><button class="btn btn-secondary text-wrap" id="execution-refresh">Refresh saved evidence</button></div><div class="col-12 col-md-auto d-grid"><button class="btn btn-secondary text-wrap" id="execution-import">${icon('upload')} Import new snapshot</button></div><div class="col-12 col-md-auto d-grid"><button class="btn btn-secondary text-wrap" id="execution-agreement">Review agreement</button></div><label class="form-label d-grid gap-1 mb-0 col-12 col-lg">Team <select class="form-select" id="execution-team"><option value="">All teams, including untracked</option></select></label></div><p id="execution-status" role="status" aria-live="polite"></p><div id="execution-weekly"></div><div id="execution-evidence"></div>
    <section id="execution-actions-panel"><details class="weekly-new-action border-top my-3"><summary class="py-2">Record the next action</summary><p class="hint">Capture a decision with an owner and review date. This app records the action; it does not send notifications or change Jira.</p><form id="execution-decision-form" class="execution-decision-form row g-3">
      <label class="form-label d-grid gap-1 mb-0 col-12 col-md-6">Action <input class="form-control" name="action" required maxlength="1000"></label><label class="form-label d-grid gap-1 mb-0 col-12 col-md-6">Owner <input class="form-control" name="owner" required maxlength="200"></label><label class="form-label d-grid gap-1 mb-0 col-12 col-md-6">Review date <input class="form-control" name="reviewDate" type="date" required></label>
      <label class="form-label d-grid gap-1 mb-0 col-12 col-md-6">Initiative <select class="form-select" name="initiative"><option value="">Whole plan</option>${(plan.initiatives || []).map(it=>`<option>${esc(it.name)}</option>`).join('')}</select></label>
      <label class="execution-rationale form-label d-grid gap-1 mb-0 col-12">Rationale / evidence <textarea class="form-control" name="rationale" required maxlength="10000" rows="3"></textarea></label><div class="col-12"><button class="btn btn-secondary text-wrap" type="submit">${icon('save')} Record action</button></div><p class="col-12" id="execution-decision-status" role="status" aria-live="polite"></p></form></details><h3 class="fs-5 text-body">Action follow-up</h3><button class="btn btn-secondary text-wrap" type="button" id="execution-reload-actions">Reload actions</button><div id="execution-decisions">Loading decisions…</div></section></section>`;

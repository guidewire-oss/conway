...
Provenance: this observation preceded the correction. Current `fitSentence` in
`app/js/report.js` compares commitment with the horizon; `verdictFor` in
`server/planning/schedule.go` evaluates target-date risk separately.
The historical reproduction command and output are retained in

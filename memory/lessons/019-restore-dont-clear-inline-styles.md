...
**Date:** 2026-08-30
**Provenance:** observed 2026-08-30 in-browser on the reference plan (user report: "the timeline shrinks when I double-click the Atlas bar"); DOM diff showed the lead bar's inline style going from `left: 27.66%; width: 47.87%` to `left: 27.66%` after a click; cause `release()`/`pointercancel()` in app/js/drag.js setting `bar.style.width = ''`; fixed in PR #55.

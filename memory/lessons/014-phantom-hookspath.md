...
`git config core.hooksPath` was set to `<local workspace>/git-hooks-core`, which does not
...
- Observed 2026-08-26: `git config core.hooksPath` → `<local workspace>/git-hooks-core`;
  `ls` → ENOENT; `ls <local workspace>` → only `org/org-chart`. Commit 8db5461 pushed to

...
var _ = Describe("ComputeRemedies", func() {
	...
	// The JS view is tested against a committed dump of this shape
	// (tests/fixtures/remedies-demo.json, regenerated by `go run ./tools/fixgen`).
	// JS cannot notice a Go field rename, so this spec is the tripwire: it fails
	// when the fixture no longer describes this package — the signal to
	// regenerate, exactly like the schedule fixture's twin spec.
	It("still matches the committed remedies fixture the JS view is tested against", func() {
		blob, err := os.ReadFile(filepath.Join("..", "..", "tests", "fixtures", "remedies-demo.json"))
		Expect(err).NotTo(HaveOccurred())

...
var _ = Describe("netPodsToTeams", func() {
	It("maps roster pods to planning teams", func() {
		pods := []NetPod{
		...
		Expect(teams[0]).To(Equal(planning.Team{Name: "Alpha", Devs: 6, Pairs: true, Tracks: 3, Site: "Bengaluru"}))
		Expect(teams[1]).To(Equal(planning.Team{Name: "Beta", Devs: 4, Site: "Remote"}))
	})
})

var _ = Describe("attachPlanRoster", func() {
	It("requires a rosterId, answering 400 with the reason", func() {
		s := &server{db: &db.DB{}}
...
var _ = Describe("previewPlanInitiatives", func() {
	It("returns the network and sim a save would, without saving", func() {
		teams, inits := planning.Demo()
		xlsx := planning.WriteInitiativesXLSX(teams, inits)
		body, ct := multipartFileG("file", "initiatives.xlsx", xlsx)
		...
		Expect(resp.Initiatives).To(HaveLen(len(inits)))
		Expect(resp.Network).NotTo(BeNil())
		Expect(resp.Network.Nodes).NotTo(BeEmpty(), "the preview must carry a non-empty network")
		Expect(resp.Sim.Before.Total).NotTo(BeZero(), "the preview must carry a sim result")
		// the plan row passed in must not have been mutated/saved
		Expect(p.Initiatives).To(BeNil(), "preview must not persist")
	})
})
...
var _ = Describe("simulatePlan", func() {
	// simulatePlan must use a client-supplied initiatives override when present
	// (draft preview mode) instead of the plan's saved initiatives — without
	// this, editing levers on an unsaved draft would silently simulate the OLD
	// saved sheet instead of the one on screen.
	It("uses the client's initiatives override, not the saved sheet", func() {
		...
		Expect(json.Unmarshal(rec.Body.Bytes(), &resp)).NotTo(HaveOccurred())
		Expect(resp.Before.Initiatives).To(HaveLen(1))
		Expect(resp.Before.Initiatives[0].Name).To(Equal("Draft one"),
			"the draft override, not the saved sheet")
	})
})

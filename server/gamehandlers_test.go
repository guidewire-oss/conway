...
func testWorld() *World {
	...
	if err != nil || len(w.Pods) == 0 {
		Fail("test world load: " + err.Error())
	}
	return w
}
...
var _ = Describe("GameNewNilWorldStillFailsCorrectly", func() {
	It("behaves", func() {
		s := newTestServer()
		s.world = nil
		...
		if rec.Code != 503 {
			Fail(fmt.Sprintf("expected 503 no-world for the default game, got %d: %s", rec.Code, rec.Body.String()))
		}
		if !strings.Contains(rec.Body.String(), "no world loaded") {
			Fail(fmt.Sprintf("expected the specific 'no world loaded' error, got: %s", rec.Body.String()))
		}
	})
})

var _ = Describe("GameEndpointsFlow", func() {
	It("behaves", func() {
		s := newTestServer()
		c := auth.Claims{Sub: "t1", Roles: []string{"player"}}
		...
		s.handleGameNew(rec, httptest.NewRequest("POST", "/api/game/new", nil), c)
		var v gameView
		mustJSON(rec.Body.Bytes(), &v)
		if v.Round != 1 || v.ApLeft != 3 || len(v.Pods) == 0 {
			Fail(fmt.Sprintf("bad new view: %+v", v))
		}
		...
		mustJSON(rec.Body.Bytes(), &mv)
		if mv.View.ApLeft != 3 || len(mv.View.MovesThisRound) != 0 {
			Fail(fmt.Sprintf("unstage failed: %+v", mv))
		}
		...
		mustJSON(rec.Body.Bytes(), &res)
		if res.Report.Round != 1 || res.Report.Headline == "" {
			Fail(fmt.Sprintf("bad report: %+v", res.Report))
		}
		if res.View.Round != 2 {
			Fail(fmt.Sprintf("round should advance, got %d", res.View.Round))
		}
		if s.tmap(defaultGameID)["t1"] == nil {
			Fail("submit should publish standings")
		}
		...
		s.handleSubmit(rec, httptest.NewRequest("POST", "/api/game/submit", nil), c)
		if rec.Code != 409 {
			Fail(fmt.Sprintf("expected 409 before round 2 opens, got %d", rec.Code))
		}
		...
		if !v.Over || v.Final == nil {
			Fail(fmt.Sprintf("game should end with a final score: %+v", v))
		}
	})
})
...
var _ = Describe("TesterRoleBypassesRoundGate", func() {
	It("behaves", func() {
		s := newTestServer()
		...
		s.handleGameNew(rec, httptest.NewRequest("POST", "/api/game/new", nil), c)
		if rec.Code != 200 {
			Fail(fmt.Sprintf("tester should start a game before round 1 opens, got %d: %s", rec.Code, rec.Body.String()))
		}
		var v gameView
		mustJSON(rec.Body.Bytes(), &v)
		if v.Round != 1 {
			Fail(fmt.Sprintf("expected round 1, got %+v", v))
		}
		...
		mustJSON(rec.Body.Bytes(), &mv)
		if !mv.Ok {
			Fail(fmt.Sprintf("tester should be able to stage a move on a closed session, got error: %q", mv.Error))
		}
		...
		if rec.Code != 409 {
			Fail(fmt.Sprintf("a plain player should still be blocked before round 1 opens, got %d", rec.Code))
		}
	})
})
...
var _ = Describe("TesterSessionExcludedFromStandings", func() {
	It("behaves", func() {
		s := newTestServer()
		gid := "game-under-trial"
		...
		if rec.Code != 200 {
			Fail(fmt.Sprintf("real team should be able to start once round 1 is open, got %d: %s", rec.Code, rec.Body.String()))
		}
		...
		if _, ok := st["team-a"]; !ok {
			Fail(fmt.Sprintf("real team should appear in standings: %+v", st))
		}
		if _, ok := st["__test__:facilitator1"]; ok {
			Fail(fmt.Sprintf("tester session leaked into standings/leaderboard: %+v", st))
		}
	})
})
...
var _ = Describe("StartingLoadIsSpreadNotClustered", func() {
	It("behaves", func() {
		s := newTestServer()
		rec := httptest.NewRecorder()
		...
		mustJSON(rec.Body.Bytes(), &v)
		if len(v.Pods) < 10 {
			Fail(fmt.Sprintf("fixture regressed: only %d pods", len(v.Pods)))
		}
		lo, hi := 99.0, 0.0
		atCeiling := 0
		...
		if hi-lo < 0.5 {
			Fail(fmt.Sprintf("starting loads too clustered: min %.2f max %.2f", lo, hi))
		}
		if atCeiling > len(v.Pods)/3 {
			Fail(fmt.Sprintf("%d/%d pods pinned near ceiling — still a wall", atCeiling, len(v.Pods)))
		}
	})
})

...
func (s *server) announcementCatalog(c auth.Claims) []featureAnnouncement {
	...
	if c.Has("manager") || c.Has("admin") {
		if s.db != nil {
			features = append(features, featureAnnouncement{ID: "scrollable-timeline-v1", Title: "Read long timelines and reveal full names", Description: "Scroll through later weeks while labels stay visible. Drag the divider beside timeline labels, or use its arrow keys, to reveal longer initiative names without changing the plan.", Action: announcementAction{Type: "route", Route: "?view=plan&planView=timeline", Target: "view-timeline", Parent: "plan-btn"}})
			features = append(features, featureAnnouncement{ID: "roster-initiative-sample-v1", Title: "Start a workbook with your own teams", Description: "Attach a roster in Plan setup, then download an initiatives sample with matching team columns. Replace the example work and preview the workbook before saving.", Action: announcementAction{Type: "route", Route: "?view=plan&planView=setup", Target: "plan-init-sample", Parent: "plan-btn"}})
			features = append(features, featureAnnouncement{ID: "forecast-registration-v1", Title: "Freeze a model before future work", Description: "Register a probability estimate from completed history, then assess new predictions against its frozen fit. Find Registered models in Forecasts > Prediction history.", Action: announcementAction{Type: "route", Route: "?view=plan&planView=forecast", Target: "view-forecast", Parent: "plan-btn"}})
			features = append(features, featureAnnouncement{ID: "forecast-model-evaluation-v1", Title: "Test forecast learning on later work", Description: "Fit a simple probability estimate from earlier completed evidence, then evaluate later distinct work. See sample gaps, outcome coverage and probability errors in Prediction history.", Action: announcementAction{Type: "route", Route: "?view=plan&planView=forecast", Target: "view-forecast", Parent: "plan-btn"}})

...
func TestParseDetailedFallsBackToEpicLinkField(t *testing.T) {
	...
	if d.ParentKey != "PROJ-1" {
		t.Fatalf("ParentKey = %q, want PROJ-1 (from Epic Link field)", d.ParentKey)
	}
}

func TestParseDetailedPrefersParentOverEpicLinkField(t *testing.T) {
	c := &Client{PodField: "customfield_10026", EpicLinkField: "customfield_10008"}
	raw := []byte(`{"key":"PROJ-42","fields":{"parent":{"key":"PROJ-2"},"customfield_10008":"PROJ-1"}}`)
	d := c.parseDetailed(raw, "customfield_10014")
	if d.ParentKey != "PROJ-2" {
		t.Fatalf("ParentKey = %q, want PROJ-2 (native parent wins)", d.ParentKey)
	}
}

func TestParseDetailedNoEpicLinkFieldConfigured(t *testing.T) {
	c := &Client{PodField: "customfield_10026"} // EpicLinkField unresolved/absent
	raw := []byte(`{"key":"PROJ-42","fields":{"customfield_10008":"PROJ-1"}}`)
	d := c.parseDetailed(raw, "customfield_10014")
	if d.ParentKey != "" {
		t.Fatalf("ParentKey = %q, want empty when no Epic Link field is configured", d.ParentKey)
	}
}

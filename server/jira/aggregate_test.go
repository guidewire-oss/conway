...
var _ = Describe("AggregateStatsAndEdges", func() {
	It("behaves", func() {
		c := day("2026-01-01")
		issues := []Issue{
		...
		if a.WipCount != 1 {
			Fail(fmt.Sprintf("Alpha wip = %d, want 1", a.WipCount))
		}
		if a.ResolvedCount180d != 3 { // A-1, A-2, and the A-1 blocker dup-key resolved
			Fail(fmt.Sprintf("Alpha resolved = %d, want 3", a.ResolvedCount180d))
		}
		b := stats["Beta"]
		if b.ResolvedCount180d != 1 { // epic excluded
			Fail(fmt.Sprintf("Beta resolved = %d, want 1 (epic excluded)", b.ResolvedCount180d))
		}

		if len(edges) != 1 || edges[0].From != "Alpha" || edges[0].To != "Beta" || edges[0].Count != 1 {
			Fail(fmt.Sprintf("edges = %+v, want one Alpha->Beta", edges))
		}
	})
})

var _ = Describe("AggregateWipOnlyPodGetsDefaultEntry", func() {
	It("behaves", func() {
		stats, _ := Aggregate([]Issue{{Key: "X-1", Pod: "Gamma", IssueType: "Story", Created: day("2026-01-01"), StatusCat: "indeterminate"}}, WipModeLeaf)
		g, ok := stats["Gamma"]
		if !ok {
			Fail("wip-only pod Gamma should still get a stats entry")
		}
		if g.WipCount != 1 || g.ResolvedCount180d != 0 || g.ThroughputPerWeek != 0 {
			Fail(fmt.Sprintf("Gamma default entry wrong: %+v", g))
		}
	})
})
...
var _ = Describe("AggregateWipExcludesBacklog", func() {
	It("behaves", func() {
		...
		if k.WipCount != 1 {
			Fail(fmt.Sprintf("Delta wip = %d, want 1 (backlog issues must not count as wip)", k.WipCount))
		}
	})
})
...
var _ = Describe("AggregateWipModeLeafExcludesEpics", func() {
	It("behaves", func() {
		c := day("2026-01-01")
		stats, _ := Aggregate([]Issue{
		...
		d := stats["Delta"]
		if d.WipCount != 2 {
			Fail(fmt.Sprintf("Delta wip = %d, want 2 (epic excluded, both children counted)", d.WipCount))
		}
	})
})
...
var _ = Describe("AggregateWipModeEpicOrParentless", func() {
	It("behaves", func() {
		c := day("2026-01-01")
		stats, _ := Aggregate([]Issue{
			{Key: "E-1", Pod: "Delta", IssueType: "Epic", Created: c, StatusCat: "indeterminate"},
		...
		d := stats["Delta"]
		if d.WipCount != 2 {
			Fail(fmt.Sprintf("Delta wip = %d, want 2 (epic E-1 + parentless D-3; D-1/D-2 folded into their epic)", d.WipCount))
		}
	})
})

var _ = Describe("PodAlias", func() {
	It("behaves", func() {
		if podOf("Moose Factory") != "MooseFactory" {
			Fail("alias not applied")
		}
	})
})

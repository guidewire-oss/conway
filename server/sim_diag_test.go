...
// print the WIP trend + final-score spread. Verbose output rides GinkgoWriter
// (shown with go test -v, or ginkgo -v).
var _ = Describe("sim diagnosis (headless balance)", func() {
	It("runs the strategies and reports the score spread", func() {
		w := testWorld()
		...
		strategies := map[string]func(g *game.Game) []game.Move{
			"do-nothing": func(_ *game.Game) []game.Move { return nil },
			...
			"optimal-line": func(g *game.Game) []game.Move {
				...
				case 1:
					return []game.Move{
						{Lever: "wipCap", Pod: "Cascade", CapX: 0.8}, {Lever: "wipCap", Pod: "Delta", CapX: 0.8},
						{Lever: "wipCap", Pod: "Beacon", CapX: 0.8}, {Lever: "wipCap", Pod: "Atlas", CapX: 0.8},
						{Lever: "hire", Pod: "Delta"},
					}
				case 2:
					return []game.Move{
						{Lever: "wipCap", Pod: "Harbor", CapX: 0.8}, {Lever: "wipCap", Pod: "Capitola", CapX: 0.8},
						{Lever: "wipCap", Pod: "Beta", CapX: 0.8},
						{Lever: "innovate", Pod: "Cascade", Flavor: "holistic"}, // 2 AP
						{Lever: "commit", Pod: "Cascade", DueRound: g.Round + 1},
					}
				case 3:
					return []game.Move{
						{Lever: "hygieneSprint", Pod: "Cascade"}, {Lever: "hygieneSprint", Pod: "Delta"},
						{Lever: "interfaceInvest", From: "Fjord", To: "Cascade"},
						{Lever: "interruptPolicy", Pod: "Krakow", Model: "followsun"},
						{Lever: "wipCap", Pod: byRho(g)[0], CapX: 0.8},
						{Lever: "commit", Pod: "Delta", DueRound: g.Round + 1},
					}
				default:
					h := byRho(g)
					return []game.Move{
		...
		for name, plan := range strategies {
			...
			fmt.Fprintf(GinkgoWriter, " round | totalWIP | avgRho | delivered | cost | score\n")
			for r := 1; r <= rounds; r++ {
				for _, m := range plan(g) {
					if ok, msg := game.PlanMove(g, m); !ok {
						fmt.Fprintf(GinkgoWriter, "   (move rejected: %s %s — %s)\n", m.Lever, m.Pod, msg)
					}
				}
				...
				fmt.Fprintf(GinkgoWriter, "   %d   |  %5d   |  %4.2f  |  %7.1f  | %5.1f | %5.1f\n", rep.Round, totWip, sumRho/n, rep.ValueDelivered, rep.CostFn, rep.Score.Total)
			}
			sc, ep := game.FinalScore(g)
			results[name] = fin{sc, ep}
			fmt.Fprintf(GinkgoWriter, " FINAL score=%.1f  (roi=%.0f trust=%.0f delight=%.0f team=%.0f innov=%.0f) | EPILOGUE=%.1f (morale=%.2f ktloShare=%.2f)\n",
		...
		for _, n := range names {
			...
			fmt.Fprintf(GinkgoWriter, "  %-18s  %.1f\n", n, s)
		}
		fmt.Fprintf(GinkgoWriter, "  --> spread between best and worst: %.1f points\n", hi-lo)

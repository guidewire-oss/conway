...
# TOML has no escape-free form for arbitrary text, so values are escaped before
# they are written. A role description containing a double quote, or instructions
# containing a backslash, previously produced a file Codex could not parse — and
# nothing noticed, because *generating* it succeeded.
toml_escape() { # one-line value: escape backslashes and quotes, fold newlines
  printf '%s' "$1" | sed -e 's/\\/\\\\/g' -e 's/"/\\"/g' | tr '\n' ' '
}

# A basic multi-line string ends at the first unescaped """, so inside the block
# only backslashes and that delimiter need escaping; newlines are meaningful.
toml_escape_block() {
...
name = "$(toml_escape "$AGENT_NAME")"
description = "$(toml_escape "$DESCRIPTION")"
${MODEL_LINE}sandbox_mode = "$(toml_escape "$SANDBOX_MODE")"
developer_instructions = """
$(toml_escape_block "$BODY")
"""
EOF

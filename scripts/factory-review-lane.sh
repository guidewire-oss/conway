...
case "$CMD" in
  ...
  enable)
    ...
    if [ ! -f "$ROOT/factory.yaml" ]; then
      echo "review lane: no factory.yaml in $ROOT — run 'factory init' first." >&2
    ...
    if [ -f "$WORKFLOW" ] && ! grep -qF 'factory review-lane' "$WORKFLOW" 2>/dev/null; then
    ...
    {
      printf '# Managed by: factory review-lane. Remove with: ./factory review-lane disable\n'
